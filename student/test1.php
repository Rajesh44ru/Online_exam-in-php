			...
			<?php
                  ...
                  $row=mysqli_fetch_assoc($result);
                   echo $row['ques'];
					...
					$quizes = array(
						"$row[ques]" => 
							array("Me", "Myself", "Toyota", "All of the Above"),

						
					);

					echo '<h1>Kwon banega karod pati?</h1>';
					//iterating through available questions
					foreach ($quizes as $key => $value) {
						echo '<br>';
						echo '<div class="card">';
						echo '<div class="card-header">';
						echo '<strong>Question - </strong>'. $key;
						echo '</div>';
					...
					echo '<button type="button" class="btn btn-warning">Next</button>';
					echo '</div>';
			?>
